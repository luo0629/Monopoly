...
**抽象命令角色 (Command)**: `Command`抽象基类

- 定义所有具体命令的公共接口
- 声明执行和撤销操作的抽象方法
- 位置: `business/commands.py` 第8行
- 核心方法:
  - `execute()`: 执行命令操作
  - `undo()`: 撤销命令操作
...
- `PurchasePropertyCommand` (第17行): 购买房产命令
  - 封装购买房产的操作逻辑
  - 支持撤销购买操作（退还房产、返还金币）
  - 记录购买前的状态信息
- `UpgradePropertyCommand` (第50行): 升级房产命令
  - 封装房产升级的操作逻辑
  - 支持撤销升级操作（降级房产、返还升级费用）
  - 记录升级前的房产等级
- `PayTaxCommand` (第83行): 支付税金命令
  - 封装税金支付的操作逻辑
  - 支持撤销支付操作（返还税金）
  - 记录支付的税金数额
- `MovePlayerCommand` (第108行): 移动玩家命令
  - 封装玩家移动的操作逻辑
  - 支持撤销移动操作（恢复原位置）
  - 记录移动前的位置和奖励信息

**调用者角色 (Invoker)**: `CommandInvoker`类

- 负责调用命令对象执行请求
- 管理命令历史记录
- 提供撤销和重做功能
- 位置: `business/commands.py` 第140行
- 核心方法:
  - `execute_command(command)`: 执行命令并记录历史
  - `undo()`: 撤销上一个命令
  - `redo()`: 重做上一个撤销的命令
  - `can_undo()`: 检查是否可以撤销
  - `can_redo()`: 检查是否可以重做
...
**接收者角色 (Receiver)**: `GameManager`类

- 实际执行业务逻辑的对象
- 被命令对象调用来完成具体操作
- 位置: `business/game_manager.py`
- 相关方法:
  - 房产购买逻辑
  - 房产升级逻辑
  - 税金支付逻辑
...
- **游戏管理器中**: 所有玩家操作都通过命令模式执行
  - 购买房产: `purchase_property()` (`game_manager.py` 第323行)
  - 升级房产: `upgrade_property()` (`game_manager.py` 第329行)
  - 移动玩家: `move_player()` (`game_manager.py` 第154行)
  - 支付税金: `_handle_tax_landing()` (`game_manager.py` 第294行)

- **用户界面中**: 提供撤销/重做按钮
  - 撤销操作: `_undo_action()` (`main_window.py` 第1320行)
  - 重做操作: `_redo_action()` (`main_window.py` 第1327行)
  - 按钮状态更新: `_update_undo_redo_buttons()` (`main_window.py` 第1334行)

**命令模式的优势**:

- **操作封装**: 将请求封装为对象，便于参数化和队列化
- **撤销/重做**: 支持操作的撤销和重做功能
- **日志记录**: 可以记录操作历史，便于审计和调试
- **宏命令**: 可以组合多个命令形成复合操作
- **解耦**: 调用者和接收者之间解耦，提高系统灵活性

**命令历史管理**:

- 历史记录限制: 最多保存50条命令历史
- 内存优化: 超出限制时自动清理最旧的记录
- 状态一致性: 确保撤销/重做操作的状态一致性
